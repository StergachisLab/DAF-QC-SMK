...
# check if CT/GA deamination rates are present before plotting, if not create empty plot with message
    if region_df[region_df["strand"] == "CT"].empty and region_df[region_df["strand"] == "GA"].empty:
        print(f"No CT or GA reads found for region {chrom}:{start}-{end}. Skipping deamination and mutation rate plots.")
        dummy_plot = plt.figure(figsize=(10, 6))
        plt.text(
            0.5,
            0.5,
            "No CT or GA reads available for this region",
            fontsize=12,
            ha="center",
            va="center",
        )
