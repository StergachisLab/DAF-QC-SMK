...
corrected_bam = pysam.AlignmentFile(output_bam, "wb", template=bam)
for read in bam.fetch():
    ...
    deam_pos = table.loc[table['read_name'] == read_id, 'deamination_positions'].values[0].strip('[]').split(',')
    ...
    for pos in deam_pos:
        assert read_seq[pos] == strand[1], f"Position {pos} in read {read_id} does not match expected base {strand[0]}, found {read_seq[pos]}"
    ...
    read.set_tags([('DA', deam_pos), ('FD', deam_pos[0] if deam_pos else 0, "i"), ('LD', deam_pos[-1] if deam_pos else 0, "i"), ('ST', strand), ('MD', MD)])
